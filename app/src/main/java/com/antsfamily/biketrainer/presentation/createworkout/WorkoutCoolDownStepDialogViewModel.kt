...
@HiltViewModel
class WorkoutCoolDownStepDialogViewModel @Inject constructor() : ViewModel() {
    ...
    private val _navigateBackEvent = MutableSharedFlow<WorkoutStep.CoolDown>()
    val navigateBackEvent: SharedFlow<WorkoutStep.CoolDown> = _navigateBackEvent.asSharedFlow()

    fun initialize(step: WorkoutStep.CoolDown?) = viewModelScope.launch {
        delay(200)
        step?.let {
            _state.update {
                it.copy(
    ...
    private fun validateStep() {
        val isWorkPowerValid =
            _state.value.endPower.orZero() > 50 && _state.value.startPower.orZero() > _state.value.endPower.orZero()
